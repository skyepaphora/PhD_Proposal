...
## AR(2) and PBCs : 1 x 3 lattice

```{r setupSgram_zy}
## --- Load 
# load(file = "Data/B200_c1_AR2_mini.RData")
# load(file = "Data/B200_c1_UMP_mini.RData")
# load(file = "Data/B300_c1_UMP_mini.RData")
# load(file = "Data/B200_c3_UMP_mini.RData")
# load(file = "Data/B300_c3_UMP_mini.RData")
...
normy <- function(x){x*1e2*(x>=0)}

## --- Common z-axis limits
# zlim <- 0:1 
zlim <- c(0,max(normy(pbc$bc1$mean),normy(pbc$bc2$mean),tru$xtf))
tix <- round(seq(0,zlim[2],length.out=7))
```

```{r sgram_zy}
# # --- Prep 3 # USES SLAB WITH MAIN LINE -- ,,2
wid <- 8
png(filename = paste0("FinalPlots/PBCs_",my.type,"_B",B,"_",cc,".png"),
...
# ---------------------------------------------------------------------------------
  ...
  slab(
       # "True TFS: Example A   [AR(2)]",
       # "True TFS: Example B   [UMP]",
       "True TFS: Example C   [UMP]",
       "","Frequency","Time", yline = 2.5, cex.factor = 0.9)
  ...
  slab(
       ...
       # "1-BC: Example B   [UMP]",
       "1-BC: Example C   [UMP]",
       bquote("Average over M ="~.(M)~"simulations;  B ="~.(B)),
       "Frequency","", cex.factor = 0.9)
