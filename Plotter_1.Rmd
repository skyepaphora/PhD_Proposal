...
## --- Scaling & plot limits
bc.y$mean  <- bc.y$mean *1e5  # scale
cbc.y$mean <- cbc.y$mean*1e5

zlim.z <- range(tru$zf.true, bc.z$mean, cbc.z$mean)
zlim.y <- range(tru$yf.true, bc.y$mean, cbc.y$mean)
```
...
## ----------------------------------------------------------------------------------
      ...
      ## --- MARGINS
      par(mfrow = c(3,2), oma = c(4,1,4,3), mar = c(10,5,0,2))
  ...
  slab("CBCMTFSE:  White Noise x(t)","",
       "Frequency","Time")
  image.plot(legend.only = TRUE, legend.lab = "Evolutionary Power Spectral Density", legend.line = 1,
             zlim = zlim.z,
...
## --- Scaling & plot limits
zlim.x <- range(tru$xtf.true, bc.x$mean, cbc.x$mean)
```
