...
simp <- 1
    ...
    if(poly == 1){
      samp   <- bc1.samp
      smooth <- bc1.smooth*(simp==0) + bc1.simp.smooth*(simp==1)
...
# --- Prep 2
wid <- 8
png(filename = paste0("FinalPlots/G-YF___",
                      my.type,
                      if(my.type=="UMP"){paste0("_",cc)}else{NULL},
                      "_B",B,
                      "___",poly,"-BC.png"),
...
## --------- Sy(f) --------- ##
    ...
    legend("topright",
       legend = c(bquote(S[Y]*"(f)"),
          ...
          lwd = c(2,1,2), inset = 0.01*c(1.5,2), y.intersp = 1.25, cex = 0.9)
...
## --------- 1-BC --------- ##
    ...
    # --- My sy
    sadd(Fs, normy(syf.1), type = "l", col = skaturate$blue, lwd = 2)
    ...
    legend("topright",
       legend = c(bquote(S[Y]*"(f)"),
          ...
          lwd = c(2,2,1), inset = 0.01*c(1.5,2), y.intersp = 1.25, cex = 0.9)
...
saveGIF({  for (my.t in seq(1,N,by = 5)) {
  ...
  # --- True Plot
  splot(Fs, tru$xtf[my.t,], type = "l", ylim = ylim.tx, lwd = 2, colr = "#FCF8F8"); gc()
  ...
  gc()
    p <- p+1
    setTxtProgressBar(pb, p/length(seq(1,N,by = 5)))
