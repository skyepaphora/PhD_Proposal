...
# # --- Prep 3 
wid <- 8
png(filename = paste0("Prospectus_Defense/Plots/PBCs___",my.type,
...
# --- Prep 2
# wid <- 8
png(filename = paste0("3MT/G-YF___icassp___",
...
width = 18, height = 8*0.8, units = "in", res = 200, pointsize = 10, bg="transparent")
...
sg <- sg.1; syf <- syf.1
    ...
    splot(t,
          ...
          ylim = glim,
          labs = 0, lwd = 3, lty = 2, colr = "#F5F2EF",
          xaxt = 'n', yaxt = 'n')

    # --- g
    sadd(t, normy(ag), type = "l", col = skaturate$red, lwd = 3)
    sadd(t, normy(sg), type = "l", col = "dodgerblue", lwd = 3)

    # Other lines
    abline(v = range(tb), col = skrown$B1, lty = 2, lwd = 2)
    ...
    legend(
       ...
       legend = c("g(t)",
                  bquote(hat(g)[1]*"(t)"),
                  bquote(hat(g)[2]*"(t)")),
          col = c(skrown$B2, skaturate$red, "dodgerblue"),
          lwd = c(3,3,3), lty = c(1,1,1),
        inset = 0.01*c(1.5,2), y.intersp = 1.6, cex = 2, bg = skrown$salt)
