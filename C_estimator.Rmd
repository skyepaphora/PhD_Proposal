...
load(file = "C:/Users/skyep/Documents/Research/PhD_Proposal/Data/UMP_B200_mini.RData")    # UMP
...
## --- BC
A.bc <- outer(1:N,1:N, function(l,j){-rowSums(bc$mean)[l]/rowSums(bc$mean)[j]}); diag(A.bc) <- rep(1,N)
C.bc <- solve(A.bc,rep(-1,N))

## --- CBC
A.cbc <- outer(1:N,1:N, function(l,j){-rowSums(cbc$mean)[l]/rowSums(cbc$mean)[j]}); diag(A.cbc) <- rep(1,N)
C.cbc <- solve(A.cbc,rep(-1,N))
```

```{r plot_compare, echo = FALSE, fig.dim = c(12,8)}
# total height of normalized ct function
wid <- max(ct.n) - min(ct.n)
ctn <- ct.n^2 - wid
lim <- range(ctn, C.bc, C.cbc)

par(mar = c(4,5,4,1))
splot(t,ctn, type = "l", ylim = lim, colr = skrown$smoke)
lines(C.bc , col = skaturate$blue, lwd = 2)
lines(C.cbc, col = skol$orange, lwd = 2)
slab(main = "c(t) and estimates",
     subb = bquote(hat(c)*" is the solution to Ac= 0"),
     xlab = "Time", ylab = "c(t)")

legend("top", legend = c("BC", "CBC", "c(t) [modified]"),
