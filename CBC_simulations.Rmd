...
## --- TRUE series info ------ ##
ct       <- 2 - exp((-(t-500)^2)/(2*(200^2)))                       # Modulating fn.  (C)
xf.true  <- rep(10^2, NF)                                           # Noise Spectrum  (Z)
yf.true  <- (10^4)/( 1 - 2.24*cos(2*pi*Fs) + 1.6*(cos(2*pi*Fs))^2 ) # AR(2) Spectrum  (Y)

ztf.true <- outer(rep(1,N), xf.true)                                # Noise Spectrogram 
ytf.true <- outer(rep(1,N), yf.true)                                # AR(2) Spectrogram 
xtf.true <- outer(ct^2, yf.true)                                    # UMP Spectrogram (X)
```
...
```{r}
my.type <- "noise"
trial <- cbc(type = my.type, M = 100, seed = 17, B = 200)
```
...
## --- Save! --------- ##
save.image(paste0("C:/Users/skyep/Documents/Research/PhD_Proposal/Data/",my.type,"_B",B,"_full.RData"))

rm(trial); gc() # discard individual simulations, keep stats

save.image(paste0("C:/Users/skyep/Documents/Research/PhD_Proposal/Data/",my.type,"_B",B,"_mini.RData"))
```
